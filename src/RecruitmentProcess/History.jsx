...
export default function History() {
  ...
  // Fetch transfer data with optional employee ID and date filters
  const fetchTransferData = async (empId = null, fDate = '', tDate = '') => {
    setLoading(true);
    try {
      // Build base URL
      let url = `${API_BASE_URL}/empTrsferGetDt`;
      if (empId) {
        url += `/${empId}`;
      }
      ...
      if (res.data && res.data.ovrlEmpHistryData) {
        fetchedData = res.data.ovrlEmpHistryData;
  ...
  // Initial load (all data, no filters)
  useEffect(() => {
    if (token?.token) {
      fetchTransferData();
    }
  ...
  const handleEmpChange = (id) => {
    ...
    setSelectedPlant('');
    setPage(1);
    fetchTransferData(id, fromDate, toDate);
  };
  ...
  const handleFromDateChange = (e) => {
    ...
    setFromDate(newFrom);
    setPage(1);
    fetchTransferData(selectedEmp, newFrom, toDate);
  };
  ...
  const handleToDateChange = (e) => {
    ...
    setToDate(newTo);
    setPage(1);
    fetchTransferData(selectedEmp, fromDate, newTo);
  };

  // Load employee dropdown once
  useEffect(() => {
    if (token?.token) FilterEmployee();
  }, [token?.token]);

  // Reset page when plant filter changes
  useEffect(() => {
    setPage(1);
  }, [selectedPlant]);

  // Derive unique plants from loaded transfer data (trim to avoid duplicates)
 const plantOptions = useMemo(() => {
  const plantMap = new Map(); // key: lowercased name, value: original trimmed name
  transferData.forEach(t => {
    if (t.old_plant) {
      const raw = t.old_plant;


      plantMap.set(raw.toLowerCase(), raw);
    }
    if (t.new_plant) {
      const raw = t.new_plant.trim();
      plantMap.set(raw.toLowerCase(), raw);
    }
  });
  ...
  // Apply plant filter client‑side (dates are already applied server‑side)
  ...
  const s = {
    ...
    dlBtn: {
      ...
      padding: '0 14px',
      borderRadius: '9px',
      border: `1.5px solid ${P.accentBtn}`,
      background: P.accentBtn,
      color: '#fff',
      fontSize: '12px',
      fontWeight: 700,
      cursor: 'pointer',
    ...
    tableCard: {
      background: '#fff',
      borderRadius: '16px',
      border: `1px solid ${P.colHeadBorder}`,
      boxShadow: '0 1px 4px rgba(0,0,0,0.06)',
      overflow: 'hidden',
    },
    colHeader: {
      ...
      borderBottom: `2px solid ${P.colHeadBorder}`,
      background: P.colHeadBg,
    },
    colCell: (align = 'flex-start', extra = {}) => ({
      display: 'flex',
      alignItems: 'center',
      ...
      color: P.colHeadText,
      borderRight: `1px solid ${P.colHeadBorder}`,
      ...extra,
    }),
    row: (even, hov) => ({
      display: 'grid',
      ...
      background: hov ? '#f5f3ff' : even ? '#fff' : '#fdfcff',
    ...
    pagFooter: {
      padding: '9px 18px',
      background: '#faf9ff',
      borderTop: `1.5px solid ${P.colHeadBorder}`,
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'space-between',
      gap: '8px',
    ...
    pgBtn: (active, disabled) => ({
      border: active ? 'none' : `1.5px solid ${P.colHeadBorder}`,
      ...
      gap: '3px',
      background: active ? P.accentBtn : disabled ? '#f3f4f6' : '#fff',
      color: active ? '#fff' : disabled ? '#d1d5db' : '#4b3fa0',
      boxShadow: active ? '0 2px 8px rgba(124,111,207,.35)' : 'none',
    }),
  };
        ...
        {!loading &&
          pagedData.map((t, i) => {
            ...
            return (
              <div
                key={t.case_id || i}
                style={s.row(globalIdx % 2 === 0, hoveredRow === i)}
              ...
              >
                ...
                <div style={s.cell({ gap: '10px' })}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '10px', color: '#9ca3af', marginTop: '1px', display: 'flex', alignItems: 'center', gap: '3px' }}>
                      ...
                      <Clock size={9} strokeWidth={2} />{formatDateDMY(t?.TRANSFER_DATE)}
                    </div>
                  </div>
                </div>
                ...
                <div style={s.cell({ justifyContent: 'center', borderRight: 'none' })}>
                  ...
                  >
                    ...
                    {formatDateDMY(t?.TRANSFER_DATE || 'Unknown')}
                  </span>
                </div>
              </div>
            );
        ...
        {!loading && filteredData.length > 0 && (
          <div style={s.pagFooter}>
            <span style={s.pageText}>
              Page <b style={{ color: '#374151' }}>{page}</b> of <b style={{ color: '#374151' }}>{totalPages}</b> · {PAGE_SIZE} rows/page
            </span>
            <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
              <button onClick={() => setPage(1)} disabled={page === 1} style={s.pgBtn(false, page === 1)}>
                <ChevronsLeft size={13} strokeWidth={2.2} />
              </button>
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} style={s.pgBtn(false, page === 1)}>
                <ChevronLeft size={13} strokeWidth={2.2} />
              </button>
              {pageNums().map(p => (
                <button key={p} onClick={() => setPage(p)} style={{ ...s.pgBtn(p === page, false), minWidth: '30px' }}>
                  {p}
                </button>
              ))}
              <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} style={s.pgBtn(false, page === totalPages)}>
                <ChevronRight size={13} strokeWidth={2.2} />
              </button>
              <button onClick={() => setPage(totalPages)} disabled={page === totalPages} style={s.pgBtn(false, page === totalPages)}>
