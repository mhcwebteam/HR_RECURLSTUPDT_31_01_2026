...
import { ArrowLeftIcon, BadgeIcon, BriefcaseIcon, CircleAlert, DeleteIcon, InfoIcon, PersonStandingIcon, PhoneIcon, RefreshCw, Trash2, Users } from 'lucide-react';
...
// ✅ Panel assignment modal state — added 20/08/2026, multi-row support added later
const [panelOpen, setPanelOpen] = useState(false);
const [panelRowData, setPanelRowData] = useState(null);
const [panelEntries, setPanelEntries] = useState([{ id: 1, name: '', designation: '', dept: '' }]);
const [panelSubmitting, setPanelSubmitting] = useState(false);
const [panelAssigned, setPanelAssigned] = useState({}); // caseId -> true after successful submit
...
//added it ajith 20/08/2026
    ...
    // employee-dept API — used for Transfer's designation/dept AND the new Panel modal
    ...
    // ✅ Deduped Designation list for the Panel modal dropdown (from employee-dept API)
    const uniquePanelDesignations = useMemo(() => {
        if (!Array.isArray(designationData?.employeeData)) return [];
        const seen = new Set();
        return designationData.employeeData.filter((item) => {
            if (!item?.DESIGNATION || seen.has(item.DESIGNATION)) return false;
            seen.add(item.DESIGNATION);
            return true;
        });
    }, [designationData?.employeeData]);

    // ✅ Deduped Department list for the Panel modal dropdown (from employee-dept API)
    const uniquePanelDepts = useMemo(() => {
        if (!Array.isArray(designationData?.employeeData)) return [];
        return [...new Set(designationData.employeeData.map((item) => item.DEPT).filter(Boolean))];
    }, [designationData?.employeeData]);

    // Opens the Panel modal for a given row
    const handleOpenPanel = (rowData) => {
        setPanelRowData(rowData);
        setPanelEntries([{ id: Date.now(), name: '', designation: '', dept: '' }]);
        setPanelOpen(true);
    };

    const handleClosePanel = () => {
        setPanelOpen(false);
        setPanelRowData(null);
        setPanelEntries([{ id: Date.now(), name: '', designation: '', dept: '' }]);
    };

    // Adds another blank Panel Name / Designation / Dept row
    const handleAddPanelEntry = () => {
        setPanelEntries(prev => [...prev, { id: Date.now(), name: '', designation: '', dept: '' }]);
    };

    // Removes a row (keeps at least one row present)
    const handleRemovePanelEntry = (id) => {
        setPanelEntries(prev => (prev.length > 1 ? prev.filter((e) => e.id !== id) : prev));
    };

    // Generic field update (name/designation/dept) for a given row
    const handlePanelEntryChange = (id, field, value) => {
        setPanelEntries(prev => prev.map((e) => (e.id === id ? { ...e, [field]: value } : e)));
    };

    // When Designation changes on a row, auto-fill the matching Department (still editable after)
    const handlePanelEntryDesignationChange = (id, value) => {
        const found = designationData?.employeeData?.find((item) => item.DESIGNATION === value);
        setPanelEntries(prev => prev.map((e) =>
            e.id === id ? { ...e, designation: value, dept: found?.DEPT || e.dept } : e
        ));
    };

    const handlePanelSubmit = async () => {
        const incomplete = panelEntries.some((e) => !e.name.trim() || !e.designation || !e.dept);
        if (incomplete) {
            Swal.fire({
                icon: 'warning',
                title: 'Incomplete Data',
                text: 'Please fill Panel Name, Designation & Department for every row',
                timer: 2400,
        ...
        const caseId = panelRowData?.CHILD_CASEID;

        const payload = {
            case_id: caseId || '',
            panels: panelEntries.map((e) => ({
                panel_name: e.name.trim(),
        ...
        try {
            // ⚠️ TODO: confirm the real endpoint for saving panel assignment — this is a placeholder.
            const response = await axiosInstance.post(
                `${API_BASE_URL}/panel-assign-store`,
                payload,
                {
                    headers: {
                        Authorization: `Bearer ${userToken.token}`,
            ...
            await Swal.fire({
                ...
                text: response?.data?.message || 'Panel details saved successfully',
                timer: 1500,
                showConfirmButton: false,
            });

            setPanelAssigned(prev => ({ ...prev, [caseId]: true }));
            handleClosePanel();
        } catch (error) {
            console.error('Panel assign error:', error);
            Swal.fire({
                icon: 'error',
                title: 'Failed!',
...
const handleActionTypeChange = (caseId, value, rowData) => {
    ...
    if (value === 'New') {
        ...
        // ✅ Panel assignment validation for "New" action — added
        if (!panelAssigned[caseId]) {
            Swal.fire({
                icon: 'warning',
                title: 'Panel Required',
                text: 'Please select the assign panel before proceeding.',
                confirmButtonColor: '#1e40af',
            });
            // Reset selection
            setActionTypeSelections(prev => ({ ...prev, [caseId]: '' }));
...
// ✅ New Panel column — added 20/08/2026
{
    field: 'PANEL',
    headerName: 'Panel',
    flex: 1,
    ...
    renderCell: (params) => {
        const caseId = params.row.CHILD_CASEID;
        const isAssigned = panelAssigned[caseId];
        return (
            <Button
                variant="outlined"
                size="small"
                onClick={() => handleOpenPanel(params.row)}
                startIcon={<span style={{ fontSize: '11px' }}>{isAssigned ? '✅' : '🧑‍⚖️'}</span>}
                sx={{
                    fontSize: '11px',
                    padding: '4px 10px',
                    textTransform: 'none',
                    borderRadius: '6px',
                    fontWeight: 700,
                    backgroundColor: isAssigned ? '#f0fdf4' : '#f8faff',
                    borderColor: isAssigned ? '#22c55e' : '#93c5fd',
                    color: isAssigned ? '#15803d' : '#1e2022d8',
                    '&:hover': {
                        backgroundColor: isAssigned ? '#dcfce7' : '#eff6ff',
                        borderColor: isAssigned ? '#16a34a' : '#60a5fa',
                    },
                }}
            >
                {isAssigned ? 'Panel Set' : 'Assign Panel'}
            </Button>
        );
    },
},
...
/>
  ...
  <Button 
    ...
    ✓ Confirm Transfer
            ...
            {/* ✅ Panel Assignment Modal — added 20/08/2026, multi-row support added later */}
            <Modal open={panelOpen} onClose={handleClosePanel}>
              <Box sx={{
                position: 'absolute', top: '50%', left: '50%',
                transform: 'translate(-50%, -50%)',
                width: 760, maxWidth: '94%', maxHeight: '90vh',
                bgcolor: '#f8fafc', borderRadius: '14px',
                boxShadow: '0 20px 60px rgba(0,0,0,0.18)',
                overflow: 'hidden', display: 'flex', flexDirection: 'column',
              }}>
                ...
                <Box sx={{
                  ...
                  px: 2.5, py: 1.6, display: 'flex', alignItems: 'center',
                  justifyContent: 'space-between', flexShrink: 0,
                }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2 }}>
                    <Box sx={{
                    ...
                    <Box>
                      <Typography sx={{ fontWeight: 700, fontSize: '15px', color: '#fff', lineHeight: 1.2 }}>
                        Assign Interview Panel
                      </Typography>
                      <Typography sx={{ fontSize: '11.5px', color: 'rgba(255,255,255,0.75)' }}>
                        Case ID: <span style={{ color: '#ccfbf1', fontWeight: 600 }}>{panelRowData?.CHILD_CASEID || 'N/A'}</span>
                      </Typography>
                    </Box>
                  </Box>
                  <IconButton onClick={handleClosePanel} size="small" sx={{
                    color: '#fff', bgcolor: 'rgba(255,255,255,0.14)',
                    '&:hover': { bgcolor: 'rgba(255,255,255,0.24)' }, flexShrink: 0,
                  }}>
                    <CloseIcon sx={{ fontSize: 18 }} />
                  </IconButton>
                </Box>

                {/* BODY — scrollable list of panel rows */}
                <Box sx={{
                  p: 2.5, overflowY: 'auto', flex: 1,
                  ...
                  {panelEntries.map((entry, index) => (
                    <Box
                      key={entry.id}
                      sx={{
                        bgcolor: '#fff', borderRadius: '10px', border: '2px solid #e2e8f0',
                        overflow: 'hidden', flexShrink: 0,
                      }}
                    >
                      {/* Row header strip */}
                      <Box sx={{
                        px: 1.6, py: 0.7, background: 'linear-gradient(90deg, #f0fdfa, #ccfbf1)',
                        borderBottom: '1px solid #99f6e4',
                        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                      }}>
                        <Typography sx={{ fontSize: '10.5px', fontWeight: 700, color: '#0f766e', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                          Panel Member {index + 1}
                        </Typography>
                        {panelEntries.length > 1 && (
                          <IconButton
                            size="small"
                            onClick={() => handleRemovePanelEntry(entry.id)}
                            sx={{
                              width: 22, height: 22, color: '#dc2626',
                      ...
                      {/* Row fields — Panel Name / Designation / Department side by side */}
                      <Box sx={{ p: 1.6, display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
                        {/* Panel Name */}
                        <Box sx={{ flex: '1 1 200px', minWidth: 180 }}>
                          <Typography sx={{ fontSize: '11px', fontWeight: 600, color: '#475569', mb: 0.5 }}>
                            Panel Name <span style={{ color: '#ef4444' }}>*</span>
                          </Typography>
                          <TextField
                            fullWidth
                            size="small"
                            value={entry.name}
                            onChange={(e) => handlePanelEntryChange(entry.id, 'name', e.target.value)}
                            placeholder="Enter panel / interviewer name"
                            sx={{
                              '& .MuiOutlinedInput-root': {
                                bgcolor: '#f8fafc', borderRadius: '8px', fontSize: '12px', height: '36px',
                                '& fieldset': { borderColor: '#cbd5e1' },
                        ...
                        <Box sx={{ flex: '1 1 180px', minWidth: 160 }}>
                          ...
                          <FormControl fullWidth size="small">
                            <Select
                              ...
                              onChange={(e) => handlePanelEntryDesignationChange(entry.id, e.target.value)}
                              displayEmpty
                              sx={{
                                height: '36px', fontSize: '12px', borderRadius: '8px', bgcolor: '#f8fafc',
                            ...
                            >
                              ...
                              {uniquePanelDesignations.map((item, idx) => (
                                <MenuItem key={`${entry.id}-${item.DESIGNATION}-${idx}`} value={item.DESIGNATION} sx={{ fontSize: '12px' }}>
                                  {item.DESIGNATION}
                                </MenuItem>
                              ))}
                            </Select>
                        ...
                        <Box sx={{ flex: '1 1 180px', minWidth: 160 }}>
                          ...
                          <FormControl fullWidth size="small">
                            <Select
                              value={entry.dept}
                              onChange={(e) => handlePanelEntryChange(entry.id, 'dept', e.target.value)}
                              displayEmpty
                              sx={{
                                height: '36px', fontSize: '12px', borderRadius: '8px', bgcolor: '#f8fafc',
                                '& .MuiOutlinedInput-notchedOutline': { borderColor: '#cbd5e1' },
                            ...
                            >
                              ...
                              {uniquePanelDepts.map((dept, idx) => (
                                <MenuItem key={`${entry.id}-${dept}-${idx}`} value={dept} sx={{ fontSize: '12px' }}>
                                  {dept}
                                </MenuItem>
                              ))}
                            </Select>
                  ...
                  {/* + Add button */}
                  <Button
                    onClick={handleAddPanelEntry}
                    fullWidth
                    startIcon={<span style={{ fontSize: '14px' }}>➕</span>}
                    sx={{
                      py: 1, borderRadius: '10px', fontSize: '12.5px', fontWeight: 700,
                      textTransform: 'none', color: '#0f766e',
                      border: '2px dashed #5eead4', bgcolor: '#f0fdfa',
                      '&:hover': { bgcolor: '#ccfbf1', borderColor: '#14b8a6' },
                    }}
                  >
                    Add Panel Member
                  </Button>
                ...
                {/* FOOTER */}
                <Box sx={{ px: 2.5, py: 1.4, bgcolor: '#fff', borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'flex-end', gap: 1.2, flexShrink: 0 }}>
                  <Button
                    onClick={handleClosePanel}
                    size="small"
                    startIcon={<CloseIcon sx={{ fontSize: 16 }} />}
                    sx={{ px: 2.2, borderRadius: '8px', fontSize: '12px', fontWeight: 600, color: '#dc2626', border: '1px solid #fecaca', bgcolor: '#fef2f2', textTransform: 'none', '&:hover': { bgcolor: '#fee2e2', borderColor: '#ef4444' } }}
                  >
                    Cancel
                  </Button>
                  <Button
                    onClick={handlePanelSubmit}
                    disabled={panelSubmitting || panelEntries.some((e) => !e.name.trim() || !e.designation || !e.dept)}
                    size="small"
                    sx={{
                      px: 3, borderRadius: '8px', fontSize: '12px', fontWeight: 700,
                      background: 'linear-gradient(135deg, #0f766e 0%, #14b8a6 100%)',
                      color: '#fff', boxShadow: '0 3px 12px rgba(15,118,110,0.35)',
                      '&:hover': { boxShadow: '0 5px 18px rgba(15,118,110,0.5)' },
                      '&:disabled': { background: '#e2e8f0', color: '#94a3b8', boxShadow: 'none' },
                    }}
                  >
                    {panelSubmitting ? <CircularProgress size={14} sx={{ color: '#fff' }} /> : `✓ Submit (${panelEntries.length})`}
                  </Button>
                </Box>
              </Box>
            </Modal>
