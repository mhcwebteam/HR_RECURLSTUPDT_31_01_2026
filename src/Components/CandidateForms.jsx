...
const CandidateForms = () => {
    ...
    const [isViewOnly, setIsViewOnly] = useState(false);
    const [windowWidth, setWindowWidth] = useState(window.innerWidth);
    const [chatOpen, setChatOpen] = useState(false); // Simple chat state
    ...
    return (
        <div style={styles.container}>
            ...
            <div style={{ width: '100%', padding: 'clamp(4px, 1vw, 8px)' }}>
                ...
                <div style={styles.header}>
                    <div style={styles.headerBox}>
                        ...
                        >
                            <ChatIcon sx={{ fontSize: 'clamp(16px, 2vw, 20px)' }} />
            ...
            {/* AI Chat Component - Simple Two-Way Communication */}
            <AIChat 
                open={chatOpen} 
                onClose={() => setChatOpen(false)}
                caseId={userToken?.Emp_Id}
                userRole="candidate"
                senderId={userToken?.Emp_Id || 'candidate_001'}
                hrName="HR Team"
                candidateName={userToken?.Name || 'Candidate'}
            />
        </div>
    );
};
