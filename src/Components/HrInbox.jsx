...
import {
  ...
  ClipboardCheck, FileSignature, BotMessageSquare
} from 'lucide-react';
...
const HrInbox = () => {
  ...
  console.log("gggggggggggg", selectedCandidateEmail);
                  ...
                  <thead>
                    <tr className="bg-gradient-to-r from-gray-100 via-blue-50 to-gray-100 border-b-2 border-gray-300">
                      {[
                        ...
                        { key: 'aiAssistance', label: 'AI Assistance', width: 'w-24' }
                      ].map((col) => (
                        <th
                          key={col.key}
                      ...
                      >
                        ...
                        <td className={tdStyle}>
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-semibold transition-all duration-200 shadow-sm hover:shadow-md hover:scale-105 cursor-pointer bg-gradient-to-r from-blue-100 to-indigo-200 text-indigo-700 hover:from-blue-200 hover:to-indigo-300 border border-indigo-200"
                          >
  ...
  <button
    onClick={() => {
      // Only allow chat if not Actions
      if (!row.Recruit_Process?.toLowerCase().includes('actions')) {
        openChatWithCandidate(row.Child_CaseId, row.EMAIL || '');
      }
    }}
    disabled={row.Recruit_Process?.toLowerCase().includes('actions')}
    className={`inline-flex items-center justify-center p-1.5 rounded-full transition-colors duration-200 border ${
      row.Recruit_Process?.toLowerCase().includes('actions')
        ? 'border-gray-200 opacity-50 cursor-not-allowed'
        : 'border-gray-200 hover:border-blue-400 hover:bg-blue-100 cursor-pointer'
    }`}
    title={row.Recruit_Process?.toLowerCase().includes('actions') ? 'Not available for Actions' : 'Chat with AI Assistant'}
  >
    ...
    <div className={`bg-gradient-to-br ${colors.bgGradient} rounded-xl p-2 border-2 ${colors.border} ${colors.hoverBorder} shadow-md hover:shadow-xl ${colors.hoverShadow} transition-all duration-300 transform hover:-translate-y-1 hover:scale-105 cursor-pointer group`}>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className={`bg-gradient-to-br ${colors.iconBg} rounded-lg p-1 shadow-sm flex-1 text-center`}>
          <p className={`text-base font-bold ${colors.text}`}>{value}</p>
        </div>
